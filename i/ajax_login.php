...
else if($fmdo=='reg')
{
	...
	if($step == 1)
	{
		...
		if($dopost=='ajax_reg')
		{
			...
			$inQuery = "INSERT INTO `#@__member` (`mtype` ,`userid` ,`pwd` ,`uname` ,`sex` ,`rank` ,`money` ,`scores` ,
			`matt`, `spacesta` ,`face`,`safequestion`,`safeanswer` ,`jointime` ,`joinip` ,`logintime` ,`loginip` )
		   VALUES ('$mtype','$userid','$pwd','$user_uname','$sex','10','$dfmoney','$dfscores',
		   '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
			if($dsql->ExecuteNoneQuery($inQuery))
			{
				...
				//写入其它默认数据
				$dsql->ExecuteNoneQuery("INSERT INTO `#@__member_flink`(mid,title,url) VALUES('$mid',' ','http://www.dedecms.com'); ");
